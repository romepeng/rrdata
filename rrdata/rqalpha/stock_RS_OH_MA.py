...
依赖：stock_day_hfq的日行情->rrdata(pg),stock_day_hfq
数据库： postgresql12
...
from rrdata.utils.rqDate_trade import  (trade_date_sse,rq_util_get_trade_range,
...
def stock_RT_HH_MA_pre(table_name='stock_day_bfq_adj', N=2):
    ...
    df_i = df.set_index(['trade_date','symbol'])
    print(df_i)
    ...
    #data.to_pickle('/home/rome/.rrdata/data/stock_RT_HH_MA_pre.pkl')
    RrdataDSave(table_name="stock_RT_HH_MA_pre",if_exists='replace').save(data)
    return data 
...
def stock_RT_HH_MA_last(table_name='stock_day_adj_fillna', N=250):
    ...
    logging.info(len(data))
    logging.info(f'\n {data.head()}')
    #data.to_pickle('/home/rome/.rrdata/data/stock_RT_HH_MA.pkl')
...
def stock_RS_OH_MA_new():
    ...
    """
    #df = pd.read_pickle('/home/rome/.rrdata/data/stock_RT_HH_MA_pre.pkl')
    ...
    if (trade_date > df_last_date): 
        ...
        df_name = read_data_from_pg('stock_belong_swl',client_rrdata)[['code', 'name', 'swl_L1', 'swl_L2','swl_L3']]
