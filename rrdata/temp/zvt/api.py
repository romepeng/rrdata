...
from rrdata import rrsdk_env
from rrdata.utils.rqParameter import IntervalLevel
from rrdata.common.context import rrsdk_context
from rrdata.common.schema import Mixin, TradableEntity
from rrdata.utils.pd_utils import pd_is_not_null, index_df
from rrdata.utils.time_utils import to_pd_timestamp
...
def _get_db_name(data_schema: DeclarativeMeta) -> str:
    ...
    :return: db name
    """
    for db_name, base in rrsdk_context.dbname_map_base.items():
        if issubclass(data_schema, base):
            return db_name


def get_db_engine(
    provider: str, db_name: str = None, data_schema: object = None, data_path: str = rrsdk_env["data_path"]
) -> Engine:
    """
    ...
    db_engine = rrsdk_context.db_engine_map.get(engine_key)
    if not db_engine:
        db_engine = engine(db_name=db_name)
        #sqlite3:  create_engine("sqlite:///" + db_path, echo=False)
        rrsdk_context.db_engine_map[engine_key] = db_engine
    return db_engine
...
def get_schemas(provider: str) -> List[DeclarativeMeta]:
    ...
    """
    schemas = []
    for provider1, dbs in rrsdk_context.provider_map_dbnames.items():
        if provider == provider1:
            for dbname in dbs:
                schemas1 = rrsdk_context.dbname_map_schemas.get(dbname)
                if schemas1:
                    schemas += schemas1
    return schemas
...
def get_db_session(provider: str, db_name: str = None, data_schema: object = None, force_new: bool = False) -> Session:
    ...
    session = rrsdk_context.sessions.get(session_key)
    if not session:
        session = get_db_session_factory(provider, db_name, data_schema)()
        rrsdk_context.sessions[session_key] = session
    return session
...
def get_db_session_factory(provider: str, db_name: str = None, data_schema: object = None):
    ...
    session_key = "{}_{}".format(provider, db_name)
    session = rrsdk_context.db_session_map.get(session_key)
    if not session:
        session = sessionmaker()
        rrsdk_context.db_session_map[session_key] = session
    return session
...
def get_entity_schema(entity_type: str) -> Type[TradableEntity]:
    ...
    """
    return rrsdk_context.tradable_schema_map[entity_type]
...
def get_schema_by_name(name: str) -> DeclarativeMeta:
    ...
    :return: schema
    """
    for schema in rrsdk_context.schemas:
...
def get_entities(
    ...
    if not entity_schema:
        entity_schema = rrsdk_context.tradable_schema_map[entity_type]
...
if __name__ == "__main__":
    #print(get_entities(entity_type="block"))  #TODO
    #print(_get_db_name("em"))
    logger.info(get_db_engine("em","rrdata"))
    logger.info(get_schema_by_name("stock"))
    logger.debug(get_entity_ids())
